#!/bin/bash
## Install FFMPEG 4.2.X + OBS STUDIO 25.0.3 + NVENC on Ubuntu 18.04|19.04 64Bits
...
MakeDEB() {
    cd $source_dir
    DIST=$(lsb_release -cs)
    fpm --deb-no-default-config-files -s dir -t deb -C $FOLDER_FPM -n ffmpeg-obs-nvenc -v 25.0.3 \
        -p ffmpeg-obs-nvenc_25.0.3+${DIST}-1_amd64.deb .
    rm -rf $FOLDER_FPM
    mkdir -p /root/dist/
